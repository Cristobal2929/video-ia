...
st.set_page_config(page_title="Fénix Viral PRO | V13", layout="centered")
...
if orden := st.chat_input("Dime el tema (Lógica y Visuales 100% reales):"):
    with st.status(f"🚀 Cerebro de Acción procesando '{orden}'...", expanded=True) as status:
        ...
        # 1. Voz blindada para Streamlit Cloud
        status.write("✍️ Generando guion de alta retención...")
        with open("temp_guion.txt", "w", encoding="utf-8") as f:
            f.write(guion)
            
        subprocess.run('edge-tts --voice es-ES-AlvaroNeural --rate=-10% -f temp_guion.txt --write-media "t.mp3" --write-subtitles "t.vtt"', shell=True)
        ...
        try:
            ...
            st.error("Error al generar la voz de Microsoft. Inténtalo de nuevo.")
            st.stop()
        ...
        # Mezcla del audio original acelerado (-10%)
        ...
        if os.path.exists(v_final):
            st.success("✅ Master Finalizado con Éxito (V13 Original).")
            st.video(v_final)
