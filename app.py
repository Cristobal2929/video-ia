...
st.set_page_config(page_title="Fénix Estudio PRO | V43", layout="centered")
...
st.markdown('<div class="pro-title">FÉNIX STUDIO V43</div>', unsafe_allow_html=True)
st.markdown('<div class="pro-subtitle" style="text-align:center; color:#94A3B8; margin-bottom: 30px;">Calidad de Agencia • Subtítulos Equilibrados • HD Limpio</div>', unsafe_allow_html=True)
...
if st.button("🚀 CREAR VÍDEO VIRAL HD"):
    if len(guion_usuario.strip()) < 20:
        ...
        with st.status("🎬 Aplicando Mejoras Visuales y Renderizando...", expanded=True) as status:
            ...
            # 3. CREAR BASE VISUAL (Zoom + Oscurecimiento del 30%)
            ...
            # 4. SUBTÍTULOS CAPCUT (TAMAÑO AJUSTADO)
            status.write("🎬 Mapeando Letras...")
            ...
            for j, chunk in enumerate(chunks):
                ...
                # TAMAÑO PERFECTO: fontsize=65, borderw=5, shadowx=4, shadowy=4
            ...
            # 5. RENDER FINAL HD
            status.write("✨ Renderizando Master Final en HD...")
            ...
            if os.path.exists(v_final) and os.path.getsize(v_final) > 1000:
                st.success("🔥 ¡VÍDEO DE ALTA CALIDAD GENERADO! Subtítulos a tamaño perfecto.")
                st.video(v_final)
                st.balloons()
            else:
