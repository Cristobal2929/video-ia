...
def obtener_guion_pro(orden_usuario):
    ...
    # PROMPT MÁS HUMANO: Queremos que hable de forma natural, no como un robot.
    prompt_maestro = f"Actua como un creador de contenido viral muy carismatico. TAREA: Cuentame una historia increible sobre: {tema_limpio}. ESTRUCTURA: 1. Empieza con un dato que te vuela la cabeza. 2. Cuenta el desarrollo como si se lo explicaras a un amigo con detalles que nadie conoce. 3. REVELA EL SECRETO Y DALE UN FINAL CERRADO Y LOGICO. REGLAS: ESCRIBE TODO EN MAYUSCULAS. CERO PUNTOS. CERO COMAS. CERO TILDES. TIENE QUE SONAR MUY NATURAL Y EXPLICAR TODO HASTA EL FINAL SIN DEJAR INTRIGA. MINIMO 100 PALABRAS."

    # GUION DE EMERGENCIA MÁS NATURAL
...
st.title("🦅 Fénix Studio: Voz Humana")
...
if orden := st.chat_input("Dime tu idea (Hablaré de tú a tú):"):
    with st.status("🎬 Creando un guion más natural...", expanded=True) as status:
        ...
        status.write(f"✍️ Guion con voz humana generado.")
        ...
        status.write("✨ Exportando historia natural...")
