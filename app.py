        ...
        with st.status("💎 Fabricando Vídeo HD...", expanded=True) as status:
            ...
            if transformar_srt("t.vtt", "t.srt"):
                ...
                if clips:
                    ...
                    cmd = f"ffmpeg -y {inputs} -i \"t.mp3\" -filter_complex \"{filt}\" -map \"[outv]\" -map 5:a -c:v libx264 -pix_fmt yuv420p -profile:v baseline -level 3.0 -movflags +faststart -t {duracion}   \"{final_p}\""
