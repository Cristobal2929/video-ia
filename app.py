...
st.set_page_config(page_title="Fénix Viral PRO | V33", layout="centered")

st.markdown("""
<style>
    .stApp { background: linear-gradient(135deg, #09090b 0%, #111827 100%); color: #F8FAFC; }
    .pro-title { font-size: 38px; font-weight: 900; background: -webkit-linear-gradient(45deg, #FF0055, #FFD700); -webkit-background-clip: text; -webkit-text-fill-color: transparent; text-align: center; margin-bottom: 5px; text-transform: uppercase; letter-spacing: 2px;}
</style>
""", unsafe_allow_html=True)

st.markdown('<div class="pro-title">FÉNIX AGENCIA V33</div>', unsafe_allow_html=True)
st.markdown('<div class="pro-subtitle" style="text-align:center; color:#94A3B8; margin-bottom: 30px;">Sin Cuadrados Raros • Audio Garantizado • Zoom Cinematográfico</div>', unsafe_allow_html=True)

# 1. DESCARGA DE FUENTE UNIVERSAL
font_path = "Arial.ttf"
...
# 2. MOTOR DUAL DE VOZ (Infalible)
def generar_voz(texto, archivo_salida):
    texto_limpio = re.sub(r'[^A-Za-zÁÉÍÓÚáéíóúÑñ0-9\s.,]', '', texto)
    with open("temp_txt.txt", "w", encoding="utf-8") as f: f.write(texto_limpio)

    # Intento 1: Microsoft
    subprocess.run(["python", "-m", "edge_tts", "--voice", "es-ES-AlvaroNeural", "--rate=0%", "-f", "temp_txt.txt", "--write-media", archivo_salida])
    
    if os.path.exists(archivo_salida) and os.path.getsize(archivo_salida) > 1000:
        return True
        
    # Intento 2: Google Translate (Si Microsoft falla)
    try:
        ...
        if res.status_code == 200 and len(res.content) > 1000:
            with open(archivo_salida, 'wb') as f: f.write(res.content)
            return True
    except: pass
...
if orden := st.chat_input("Dime el tema (Calidad Agencia Activada):"):
    with st.status(f"🚀 Creando Vídeo Premium sobre '{orden}'...", expanded=True) as status:
        ...
        status.write("🧠 Lógica y Dirección preparadas.")
        clips_finales = []
        ...
        for i, esc in enumerate(escenas):
            ...
            # --- VOZ BLINDADA ---
            if not generar_voz(texto_frase, f"a_{i}.mp3"):
                st.error(f"❌ Error masivo de red en la escena {i+1}. Los servidores de voz están caídos. Inténtalo en unos minutos.")
                st.stop()
            ...
            # --- TEXTO SIN CUADRADOS (Solución Definitiva) ---
            ...
            # EL TRUCO: newline='\n' fuerza el salto de línea de UNIX y evita los cuadrados raros (\r) de Windows/Python en FFmpeg
            ...
            # --- RENDERIZADOR (Con efecto ZOOM CINEMATOGRÁFICO) ---
            font_cmd = f"fontfile='{font_path}':" if os.path.exists(font_path) else ""
            
            # Hemos añadido un ligero zoom para que los vídeos estáticos cobren vida y no aburran
            cmd_scene = f"""ffmpeg -y -stream_loop -1 -i v_{i}.mp4 -i a_{i}.mp3 -vf "scale=800:1422,zoompan=z='min(zoom+0.0015,1.5)':d=300:x='iw/2-(iw/zoom/2)':y='ih/2-(ih/zoom/2)':s=720x1280,format=yuv420p,drawtext=textfile='text_{i}.txt':fontcolor={color_sub}:fontsize=45:{font_cmd}box=1:boxcolor=black@0.6:boxborderw=15:borderw=2:bordercolor=black:line_spacing=12:x=(w-tw)/2:y=(h-th)/2,fps=30" -c:v libx264 -preset ultrafast -c:a aac -ar 44100 -ac 2 -t {duracion} s_{i}.mp4"""
        ...
        # --- UNIÓN Y MÚSICA ---
        status.write("✨ Renderizando Obra Maestra Comercial...")
        ...
        if os.path.exists(v_final):
            st.success("✅ VÍDEO AGENCIA CREADO. Limpio, dinámico y profesional.")
            st.video(v_final)
            st.balloons()
