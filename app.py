...
def crear_guion_y_keywords(tema):
    ...
    if "terror" in t:
        ...
        keys = ["abandoned house", "scary shadow", "dark hallway", "forest night"]
        return guion, keys
...
if user_input := st.chat_input("Tema del vídeo..."):
    with st.status("🚀 Editando...", expanded=True):
        # Limpieza previa
        subprocess.run("rm -f p_*.mp4 clip_*.mp4 base.mp4 t.* outro.mp4", shell=True)
        ...
        if clips:
            ...
            # Sello Final Rápido (Negro con texto)
            ...
            # Mezcla Final Simple (Voz + Subtítulos)
            est = f"Fontname=Impact,FontSize=28,PrimaryColour={ass_col},Outline=3,Alignment=2,MarginV=140"
            cmd = f'ffmpeg -y -i base.mp4 -i t.mp3 -vf "subtitles=t.srt:force_style=\'{est}\'" -c:v libx264 -preset ultrafast -shortest -c:a aac -af "volume=2.0" "{v_final}"'
