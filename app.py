...
st.set_page_config(page_title="Fénix Estudio PRO | V61", layout="centered")
...
st.markdown('<div class="pro-title">FÉNIX STUDIO V61</div>', unsafe_allow_html=True)
st.markdown('<div class="pro-subtitle" style="text-align:center; color:#94A3B8; margin-bottom: 30px;">Sync Word-by-Word • Audio de Estudio • Ken Burns FX</div>', unsafe_allow_html=True)
...
def generar_voz_inmortal(texto, codigo_voz, tl_code, dir_trabajo):
    ...
    for _ in range(2):
        # MEJORA V61: Extraemos archivo VTT con el timestamp exacto de cada palabra
...
def procesar_escena(args):
    ...
    # MEJORA V61: Ken Burns FX (zoom lento) + Viñeta
...
if st.button("🚀 CREAR VÍDEO (CEREBRO BILINGÜE V61)"):
    if len(guion_usuario.strip()) < 20 or len(tema_broll.strip()) < 2:
        ...
        with tempfile.TemporaryDirectory() as dir_trabajo:
            with st.status(f"🎬 Iniciando Motor V61 (Modo Agencia)...", expanded=True) as status:
                ...
                status.write("🎵 Aplicando EQ Micrófono Podcast y Auto-Ducking...")
                freq = 60 if "Misterio" in musica_tipo else 75
                audio_final = os.path.join(dir_trabajo, "audio_final.m4a")
                
                # MEJORA V61: Filtro de audio [v] Ecualización y Compresión para dar cuerpo a la voz + [m] Mezcla de ambiente
                ...
                tareas = []
                status.write(f"⚡ Descargando y animando {num_clips} escenas en paralelo...")
                ...
                with concurrent.futures.ThreadPoolExecutor(max_workers=3) as executor:
                    ...
                    for future in concurrent.futures.as_completed(futuros):
                        ...
                        status.write(f"✔️ Escena animada {res[0]+1} ('{res[2]}' -> '{res[3]}')")
                ...
                status.write("🎬 Generando Sync Text VTT exacto...")
                vtt_data = leer_vtt(vtt_path)
                
                subs_cmd = []
                font_cmd = f"fontfile='{font_abs}':" if os.path.exists(font_abs) else ""
                
                # MEJORA V61: Usamos los milisegundos exactos del VTT para mostrar las palabras
            ...
            if os.path.exists(v_final):
                st.success("🔥 ¡VÍDEO V61 LISTO! Calidad de Agencia conseguida con Open Source.")
                st.video(v_final)
                st.balloons()
            else:
